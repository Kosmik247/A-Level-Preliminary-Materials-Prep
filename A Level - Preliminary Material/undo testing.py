...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            self.__PreviousGrids.append([cell.GetSymbol() for cell in self.__Grid])
            self.__SymbolsLeft -= 1
            CurrentCell = self.__GetCell(Row, Column)
            if Symbol == "B" and type(CurrentCell) == BlockedCell and CurrentCell.GetSymbol() == "@":
            ...
            self.DisplayPuzzle()
            undo_move = input("Would you like to undo your last move? Y/N")
            if undo_move == "Y":
                self.undo_move()
        ...
        return self.__Score
    def undo_move(self):
        previous_grid = self.__PreviousGrids.pop()
        print(previous_grid)
        for i, symbol in enumerate(previous_grid):
            self.__Grid[i].ChangeSymbolInCell(symbol)
